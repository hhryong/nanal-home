...
CREATE TABLE APP_SEND_DATA (
  MSG_SEQ NUMERIC(11) AUTO_INCREMENT, -- 메시지 순번
  REQ_SEND_DATE CHAR(14) NOT NULL, -- 메시지 발송 일시
  PACK_UNIQUEKEY NUMERIC(11), -- APP_SEND_CONTENTS의 PK키 
  PHONE_NUM VARCHAR(12) NOT NULL, -- 수신자 전화번호
  NURI_MSG_SEQ NUMERIC(11), -- 문자로 전환될 시 누리 메시지 순번
  USER_SEQ NUMERIC(11), -- 유저가 임의로 사용 가능한 순번
  CUR_STATE NUMERIC(1) DEFAULT 0, -- 메시지 상태
  ...
  SEND_PACK_UKEY NUMERIC(11), -- 사용하지 않음
  PRIMARY KEY (MSG_SEQ)
)
...
CREATE TABLE APP_SEND_CONTENTS (
  ...
  PACK_UNIQUEKEY NUMERIC(11) AUTO_INCREMENT NOT NULL ,  -- 순번
  ...
  BTN_CNT NUMERIC(1) DEFAULT 0 , -- 버튼 개수 (현재 1개만 지원)
  ATTACHMENT VARCHAR(5000) , -- 버튼 정보 JSON 형식 
  LINK_CNT NUMERIC(1) DEFAULT 0 , -- 바로연결 버튼 개수 (현재 지원하지 않음)
  SUPPLEMENT VARCHAR(5000) , -- 바로연결 버튼 정보 (현재 지원하지 않음)
  PHONE_CNT NUMERIC(11) DEFAULT 0 NOT NULL, -- 전화번호 개수 (현재 사용 하지 않음)
  CUR_STATE NUMERIC(1) DEFAULT 0, -- 메시지 상태 (현재 사용하지 않음)
  CONSTRAINT APP_SEND_CONTENTS _PK PRIMARY KEY (PACK_UNIQUEKEY)
)
...
CREATE TABLE TEMPLATE_CODE (
  ...
  CONSTRAINT TEMPLATE_CODE _PK PRIMARY KEY (APP_GUBUN, TEMPLATE_CODE)
)
